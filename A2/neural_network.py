...
#       tf.nn.l2_loss(weights['wc3']) + tf.nn.l2_loss(weights['out'])
# cost = tf.reduce_mean(cost + lam * regularizers)
...
with tf.Session() as sess:
    ...
    for i in range(training_iters):
        ...
        for batch in range(len(trainData)//batch_size):
            ...
            opt = sess.run(optimizer, feed_dict={x: batch_x, y: batch_y, keep_prob: prob })
        loss, acc = sess.run([cost, accuracy], feed_dict={x: trainData, y: newtrain, keep_prob: prob})
#         if i%10==0:
...
#         print("Optimization Finished!")
        ...
        test_acc,test_loss = sess.run([accuracy,cost], feed_dict={x: testData,y : newtest, keep_prob: prob })
        valid_acc,valid_loss = sess.run([accuracy,cost], feed_dict={x: validData,y : newvalid, keep_prob: prob })
...
plt.title('Train, Valid and Test Loss with Adam optimizer dropout = 0.75')
...
plt.title('Train, valid and Test Accuracy with Adam optimizer dropout = 0.75')
plt.xlabel('Epochs ',fontsize=16)
plt.ylabel('Loss',fontsize=16)
plt.legend()
plt.figure()
